...
if sys.version_info >= (3, 7) or typing.TYPE_CHECKING:
    ...
    class SchemaF(Schema, typing.Generic[A]):
        ...
        def dump(
            self, obj: TOneOrMulti, many: typing.Optional[bool] = None  # type: ignore
        ) -> TOneOrMultiEncoded:
            pass
        ...
        @typing.overload
        def dumps(
            self, obj: A, many: typing.Optional[bool] = None, *args, **kwargs
        ) -> str:
            pass
        ...
        def loads(
            self,
            json_data: JsonData,  # type: ignore
...
def build_type(type_, options, mixin, field, cls):
    def inner(type_, options):
        ...
        if is_union_type(type_):
            union_types = [
                a for a in getattr(type_, "__args__", []) if a is not type(None)
            ]
...
def build_schema(
    cls: typing.Type[A], mixin, infer_missing, partial
) -> typing.Type["SchemaType[A]"]:
    Meta = type(
        "Meta",
        (),
        {
            "fields": tuple(
                field.name
                for field in dc_fields(cls)  # type: ignore
    ...
    def dump(self, obj, *, many=None):
        ...
        if many:
            ...
            dumped.update(
                _handle_undefined_parameters_safe(cls=obj, kvs={}, usage="dump")
            )
        return dumped
