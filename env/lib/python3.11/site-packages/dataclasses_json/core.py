...
from dataclasses import MISSING, fields, is_dataclass  # type: ignore
from datetime import datetime, timezone
from decimal import Decimal
from enum import Enum
from typing import Any, Collection, Mapping, Union, get_type_hints, Tuple, TypeVar, Type
...
def _user_overrides_or_exts(cls):
    ...
    try:
        cls_config = (
            cls.dataclass_json_config if cls.dataclass_json_config is not None else {}
        )
...
def _is_supported_generic(type_):
    ...
    return (
        (not_str and _is_collection(type_))
    ...
    # FIXME this is a hack to fix a deeper underlying issue. A refactor is due.
        ...
        if _is_mapping(type_) and not _is_counter(type_):
            ...
            if Ellipsis in types:
                ...
                xs = _decode_items(
                    _get_type_args(type_) or _NO_ARGS, value, infer_missing
                )
        elif _is_counter(type_):
            xs = dict(
                zip(
                    _decode_items(
                        _get_type_arg_param(type_, 0), value.keys(), infer_missing
                    ),
                    value.values(),
                )
            )
...
def _decode_items(type_args, xs, infer_missing):
    ...
    def handle_pep0673(pre_0673_hint: str) -> Union[Type, str]:
        ...
        warnings.warn(
            f"Could not resolve self-reference for type {pre_0673_hint}, "
    ...
    if _isinstance_safe(type_args, Collection) and not _issubclass_safe(
        type_args, Enum
    ):
        if len(type_args) == len(xs):
            ...
            raise TypeError(
                f"Number of types specified in the collection type {str(type_args)} "
...
def _asdict(obj, encode_json=False):
    ...
    if is_dataclass(obj):
        ...
        result = _handle_undefined_parameters_safe(
